...
minTree = foldTree f z
    where
    f x y z | x == Nothing && y == Nothing = Just y
            | x == Nothing = Just (min y (toInt z))
            | z == Nothing = Just (min y (toInt x))
            | otherwise = Just (min y (min (toInt z) (toInt x)))
    z = Nothing
...
foldRose f (Node a l) = f a (map (foldRose f) l)
...
minRose = foldRose f
    where
    f a [] = Just a
    f a l = Just (min a (minimum (map (\(Just x) -> x) l)))
